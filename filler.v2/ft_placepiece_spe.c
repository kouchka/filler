...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/09 08:46:34 by allallem          #+#    #+#             */
/*   Updated: 2018/02/10 13:42:18 by allallem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_check_x(t_filler *p, int *i, int *j)
{
	...
	if (p->piece[*i][*j] == '*' && (p->map[p->myposy + *i][p->myposx + *j] ==
		p->symbol + 32 || p->map[p->myposy + *i][p->myposx + *j] == '.'))
	{
		if (p->map[p->myposy + *i][p->myposx + *j] == p->symbol + 32)
			p->contact++;
		p->value++;
	}
	if (p->piece[*i][*j] != '\0')
